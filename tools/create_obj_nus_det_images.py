...
if __name__ == '__main__':
    ...
    agg = NuScenesAggregatorFromDetectorImages(
        ...
        data_fields = ['frame_id', 
                        ...
                        'obj_images',
                        'obj_visibility'],
        obj_aggregator = None,# ObjectAggregator(
    ...
    for idx in tqdm(range(args.begin, args.end), desc=f'[{args.rank}] Aggregating nuScenes dataset', position=2*args.rank):
        ...
        for obj_id, obj_info in obj_infos.items():
            ...
            #save images
            for i,obj_im in enumerate(obj_images):
                if obj_im is None:
                    continue
                print("=========================================================================================")
                print(i,obj_im)
                ...
                obj_info['crop_size'] = obj_im.shape[-2:]
                data_dir = obj_dir / str(i)
                os.makedirs(data_dir, exist_ok=True)
                obj_im.astype(np.float32).tofile(data_dir / f'img_crop.bin')
