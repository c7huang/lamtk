...
from lamtk.aggregation.pointcloud_aggregator import SceneAggregator
from lamtk.aggregation.waymo_aggregator import WaymoAggregator
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dataset-path',
        type=str,
        default='./data/waymo/domain_adaptation',
        required=False,
    ...
    parser.add_argument(
        '--out-dir',
        type=str,
        default='./data/lamtk/sa/waymo_da',
        required=False,
    ...
    with ctx.Pool(args.workers) as p:
        imap = p.imap(partial(worker, args.dataset_path, 'training'), np.array_split(np.arange(80), args.workers))
        for results_i in imap:
            results['scene_infos'].update(results_i['scene_infos'])
            results['frame_infos'].update(results_i['frame_infos'])
        imap = p.imap(partial(worker, args.dataset_path, 'training/unlabeled'), np.array_split(np.arange(480), args.workers))
        for results_i in imap:
            results['scene_infos'].update(results_i['scene_infos'])
            results['frame_infos'].update(results_i['frame_infos'])
        imap = p.imap(partial(worker, args.dataset_path, 'validation'), np.array_split(np.arange(20), args.workers))
        for results_i in imap:
            results['scene_infos'].update(results_i['scene_infos'])
            results['frame_infos'].update(results_i['frame_infos'])
        imap = p.imap(partial(worker, args.dataset_path, 'validation/unlabeled'), np.array_split(np.arange(120), args.workers))
        for results_i in imap:
            results['scene_infos'].update(results_i['scene_infos'])
            results['frame_infos'].update(results_i['frame_infos'])
